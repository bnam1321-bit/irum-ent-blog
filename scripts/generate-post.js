...
async function generatePost() {
    ...
    if (process.argv[2]) {
        ...
        try {
            ...
            반드시 다음 우리 병원의 핵심 전문 분야와 관련된 주제만 추천하세요:
            - 수면/코골이 클리닉 (수면장애, 코골이, 수면무호흡, 수술)
            - 어지럼증 클리닉 (이석증, 메니에르병, 전정신경염, 어지럼증 검사)
            - 난청/이명 클리닉 (돌발성 난청, 노인성 난청, 보청기 처방, 청력검사)
            - 비염/축농증 수술 클리닉 (microshaver 수술, 국소마취 시스템)
            ...
            const entTopics = [
                '심한 코골이와 수면무호흡증, 방치하면 안 되는 이유',
                '수면장애, 원인을 찾고 코골이 수술로 해결할 수 있을까?',
                '갑자기 천장이 핑 돈다면? 어지럼증 검사와 이석증 치료',
                '메니에르병과 전정신경염, 어지럼증의 다양한 원인과 대처',
                '갑자기 귀가 안 들리는 돌발성 난청, 골든타임을 지키세요',
                '노인성 난청과 정밀 청력검사, 그리고 맞춤형 보청기 처방',
                '원인 모를 이명(귀울림), 어떻게 관리하고 치료해야 할까?',
                '잘 낫지 않는 축농증과 비염, microshaver를 이용한 안전한 수술',
                '국소마취 시스템으로 더 안전해진 이비인후과 코 수술'
            ];
...
**그 다음 본문을 병원이 환자에게 직접 이야기하는 친근하고 신뢰감 있는 칼럼 형식으로 작성하세요:**

## (H2: 환자의 고민에 공감하는 따뜻한 도입부)

(도입부: "안녕하세요, 남양주 진접 이룸이비인후과입니다." 같은 병원 차원의 친근한 인사말로 시작하세요. 최근 진료실에서 자주 접하는 환자분들의 사례를 들며 공감대를 형성하세요. 200-300자)

## (원인/증상: 전문적이고 이해하기 쉬운 설명)

(의학적으로 정확하되, 진료실에서 환자에게 설명하듯 "저희 이룸이비인후과에 내원하시는 분들을 살펴보면~", "이런 증상으로 고생하시는 분들은 대체로~" 같이 병원 관점의 화법을 사용하여 원인/증상을 설명하세요)

## (치료/관리: 이룸이비인후과의 맞춤형 처방과 조언)

(병원이 직접 치료 방법을 제안하고 조언하듯 서술하세요. "저희 이룸이비인후과에서는 이렇게 치료를 돕고 있습니다", "일상생활에서 당부드리고 싶은 것은~" 등의 표현을 사용하세요)

## 진료실에서 가장 많이 듣는 질문들

**Q1: (환자들이 실제로 가장 많이 하는 질문)**
A: (병원의 의료진이 직접 대답하듯 친절하고 전문적인 답변)
...
(요점 요약 및 "언제든 편하게 내원해달라"는 자연스러운 진료 권유. 저희 이룸이비인후과의 진료 철학을 한 줄 정도 자연스럽게 녹여내세요)
...
## 새로운 글쓰기 패턴 (매우 중요)
- **주제 내용**: [이룸이비인후과의 실제 진료 경험이나 패턴] 1~2개를 반드시 포함하세요. (예: "얼마 전 저희 의원을 찾으신 40대 남성 환자분의 경우...")
- **형식**: 백과사전식 나열을 절대 피하고, 병원 공식 블로그의 정성스러운 칼럼처럼 자연스럽게 작성하세요.
- **어조**: 글 전체를 진료실에서 의료진이 환자에게 직접 설명하는 화법("저희 이룸이비인후과에서는", "환자분들을 진찰하다 보면")으로 일관되게 작성하세요. "원장으로서", "제가"와 같은 지나친 1인칭 단수 표현은 지양하고, "저희"라는 1인칭 복수 표현을 사용하세요. 3인칭 객관적 서술("~질환이다", "~이다")은 금지합니다.
